...
def process_KMT_patch(site, coords=None, quality_max=1.25, mag_err_max=0.3,
                      seeing_max=7.0, sky_max=10000,
                      name_pattern_has_site=True, date_header='MIDHJD',
                      parameters=None, q_sigma_threshold=1.0,
                      locate_date_range=None, locate_half_width=None):
    ...
    for band in ['I', 'V']:
        ...
        if coords is not None:
            ...
            if band == 'I':
                ...
                x0, y0 = coords
                print
                'Starting photometry for', prefix, 'at', (x0, y0)
                dates, seeing, roundness, bgnd, signal, flux, dflux, quality, x0, y0 = CF.photom_variable_star(
                    x0, y0, params, save_stamps=True, patch_half_width=20,
                    locate_date_range=locate_date_range,
            ...
            print
            'Photometry for', site, band, 'at', x0, y0
            ...
            'x0 y0:', x0, y0
            print
            'Nearest star', star_num, 'located at', refmags[star_num, 1], \
            refmags[star_num, 2]
            print
            'Reference flux', refflux[star_num, :]
            print
            'Reference mag', refmags[star_num, :]

            mag = 25 - 2.5 * np.log10(refflux[star_num, 0] + flux)
            mag_err = 25 - 2.5 * np.log10(
                refflux[star_num, 0] + flux - dflux) - mag

            np.savetxt(prefix + '-lightcurve.dat', np.vstack((dates, flux,
                                                              dflux, mag,
                                                              mag_err, quality,
                                                              seeing,
                                                              roundness, bgnd,
            ...
            np.savetxt(prefix + '-lightcurve-filtered.dat', np.vstack((
                                                                      dates[q],
                                                                      flux[q],
                                                                      dflux[q], \
                                                                      mag[q],
    ...
    cal.makeCMD(site + 'I', site + 'V', plot_density=False,
                IV=(median_mag['I'], median_mag['V']), RC=RC,
