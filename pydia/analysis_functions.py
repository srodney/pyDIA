...
def readflux(folder, images_file='images', seeing_file='seeing', ZP=25.0):
    ...
    siglist = []
    try:
        with open(os.path.join(folder, 'lightcurve_data.pkl'),
                  'rb') as pkl_file:
            dates, flux, eflux, mag, emag, fwhm, bgnd, signal = Pickle.load(
                pkl_file)
    except:
            ...
            for i, line in enumerate(file.split('\n')):
                if len(line) > 0 and i < len(fnames):
                    ...
                    if n == fnames[i]:
                        ...
                        print
                        'Mismatch between images and seeing files.'
                        sys.exit(0)
        ...
        for i, f in enumerate(fnames):
            ...
            emag[i, :] = mag[i, :] - ZP + 2.5 * np.log10(
                rf[:, 0] + fl[:, 0] + fl[:, 1])
        with open(os.path.join(folder, 'lightcurve_data.pkl'),
                  'wb') as pkl_file:
            Pickle.dump((dates, flux, eflux, mag, emag, fwhm, bgnd, signal),
                        pkl_file)
...
def index_good_data(mag, threshold=2.5, fraction=0.3):
    ...
    for i in range(nstars):
        nmag[:, i] = np.abs((mag[:, i] - np.median(mag[:, i]))) / mad_std(
            mag[:, i])
    ...
    for i in range(nepoch):
        if np.sum(nmag[i, :]) > ftest:
            ind[i] = False
    print
    np.sum(ind), 'epochs of data out of', nepoch, 'flagged as good'
    return ind


def plot_lightcurve_at(x, y, folder=None, params=None, error_limit=0.1,
                       tmin=None, tmax=None):
    if not (folder):
    ...
    q[emag[:, star] > np.median(emag[q, star]) + 3 * np.std(
        emag[q, star])] = False
    plt.figure()
    plt.errorbar(dates[q], mag[q, star], emag[q, star], fmt='.', ls='None',
                 ms=2, c='k')
    if tmin and tmax:
        plt.xlim((tmin, tmax))
    ...
    np.savetxt(folder + '-S%05d.mags' % (star + 1), np.vstack((dates[q],
                                                               mag[q, star],
                                                               emag[q, star],
                                                               flux[q, star],
                                                               eflux[q, star],
                                                               ...
                                                               bgnd[q],
                                                               signal[q])).T,
               fmt='%10.5f  %7.4f  %7.4f  %12.3f  %12.3f %6.2f %9.2f %9.2f')
    np.savetxt(folder + '-S%05d.all' % (star + 1), np.vstack((dates,
                                                              mag[:, star],
                                                              emag[:, star],
                                                              flux[:, star],
                                                              eflux[:, star],
...
def plotlc(files, ref, n):
    ...
    for f in files:
        if isinstance(f.flux, np.ndarray):
            ...
            dmag.append(
                f.dflux[n - 1] * 1.08574 / (f.flux[n - 1] + ref.flux[n - 1]))
    plt.errorbar(d, mag, yerr=dmag, fmt='ro')
    ax = plt.gca()
    ax.set_ylim(ax.get_ylim()[::-1])


def trend_filter(y, n, iterations=1, star_number=None, mode='mean',
                 period=None, dates=None, plotfile=None):
    def run_filter(s, templates, n, g, g1, g_inv_full, y, yt, iterations,
                   debug=False, phase=None, n_phase_bins=25):
        if s in templates:
        ...
        for i in range(iterations):
            if isinstance(phase, np.ndarray):
                for p in range(n_phase_bins):
                    q = np.where((phase > p / (1.0 * n_phase_bins)) & (
                                phase <= (p + 1) / (1.0 * n_phase_bins)))
                    ya[q] = np.mean(yy[q])
            h = np.zeros(ntemp)
            yy = y[s, :].copy()
    ...
    if star_number:
        if mode == 'minimum':
            ...
            ymean[star_number] = np.median(yc, 1).reshape(nstars, 1)[
                star_number]
    yc -= ymean
    ...
    if star_number:
        ...
        yf = run_filter(star_number, templates, n, g, g1, g_inv_full, yc, yt,
                        iterations, phase=phase, debug=False) + ymean[
                 star_number]
        if plotfile:
            plt.figure()
        ...
        for s in range(nstars):
            yf[s, :] = run_filter(s, templates, n, g, g1, g_inv_full, yc, yt,
                                  iterations)
        yf += ymean
    return yf
...
def detrend_v2(data, err_data, v1, v2):
    ...
    if len(v1) != ndates:
        print
        'Error: length of vector 1 is not equal to the number of data rows'
        return data
    if len(v2) != ndates:
        print
        'Error: length of vector 2 is not equal to the number of data rows'
        return data
...
def detrend_pca(data, err, npc=10):
    ...
    print
    'Finished computing principal components'
...
def lomb_search(dates, y, pmin, pmax, nfreq=1000, fap_threshold=1e-7,
                maxvars=100, plotfile='/dev/null'):
    nstars = y.shape[0]
    npoints = y.shape[1]
    ...
    for star in range(nstars):
        if nvars < maxvars:
            ...
            if len(qq) > 100:
                ...
                print
                y[star, qq]
                lnp = spectral.lombscargle(dates[qq], (
                            y[star, qq] - np.mean(y[star, qq])) / np.std(
                    y[star, qq]), freq)
                lnpmax = np.max(lnp)
                ...
                if fap < fap_threshold and not (
                        np.abs(p - 1.0) < 0.01) and not (
                        np.abs(p - 0.5) < 0.01) and not (
                        np.abs(p - 0.33333) < 0.01):
                    print
                    star, p, np.max(lnp), fap
                    ...
                    lfap = np.log10(fap)
                    plt.title(str(
                        star) + '     P = ' + "%6.3f" % p + ' d      $log_{10}$ FAP = ' + "%4.1f" % lfap)
                    plt.xlabel('Date')
                    plt.ylabel('Magnitude')
                    plt.subplot(3, 1, 2)
                    cycle = dates[qq] / p
                    phase = cycle - np.floor(cycle)
                    plt.plot(phase, y[star, qq], 'b.', phase + 1, y[star, qq],
                             'b.')
                    plt.ylim([ymin, ymax])
                    ax = plt.gca()
                    ax.set_ylim(ax.get_ylim()[::-1])
                    ...
                    plt.ylabel('LNP')
                    plt.savefig('var%(star)05d.png' % vars(),
...
def transit_search(dates, data, err, pmin, pmax, nfreq, min_intransit_points=4,
                   max_intransit_points=20, threshold=30, maxvars=100,
                   plotfile='/dev/null', renormalize=True, true_period=None):
    nstars = data.shape[0]
    ndates = data.shape[1]
    ...
    for i in range(n_transit_points):
        for j in range(ndates):
            ...
            C[i * ndates + j,
            j:np.min([j + i + min_intransit_points, ndates])] = 1
    ...
    for star in range(nstars):
        if nvars < maxvars:
            ...
            #                npts = ndates
            if renormalize:
                renorm = sum(w * (data[star, :].flatten() - m) ** 2) / (
                            npts - 1)
            t = np.zeros(nfreq)
            ...
            if pw[0].shape[0] and (tmax > threshold):
                ...
                print
                star, p, tmax, np.std(t), np.mean(t), (
                            tmax - np.mean(t)) / np.std(t)
                vars.append(star)
                periods.append(p)
                plt.figure()
                plt.subplot(2, 1, 1)
                ...
                cycle = dates / p
                phase = cycle - np.floor(cycle)
                plt.plot(phase, data[star, :], 'b.', phase + 1, data[star, :],
                         'b.')
                ax = plt.gca()
                ax.set_ylim(ax.get_ylim()[::-1])
...
def insert_transit(dates, mags, period, phase_start, phase_duration,
                   amplitude):
    cycle = dates / period
    phase = cycle - np.floor(cycle)
    p = np.where(
        (phase > phase_start) & (phase < phase_start + phase_duration))
    new_mags = mags
    if p[0].shape[0]:
...
def plotddp(k, dates, logp, sn, lnprob, mags, mi, eigen, ev):
    ...
    plt.title(
        str(k) + '    P = ' + str(10 ** logp[mi]) + '    S/N = ' + str(sn))
    plt.ylabel('Prob(P)')
    plt.xlabel('log P')
    ...
    plt.scatter(phase[q1], mags[q1], s=2, edgecolor='', c=colour1,
                cmap=mpl.cm.winter)
    plt.scatter(phase[q1] + 1, mags[q1], s=2, edgecolor='', c=colour1,
                cmap=mpl.cm.winter)
    print
    ph.shape
    ddd = np.mean(mags) + ev[mi, -2] * np.sin(2 * np.pi * ph) + ev[
        mi, -1] * np.cos(2 * np.pi * ph)
    plt.plot(ph, np.mean(mags) + ev[mi, -2] * np.sin(2 * np.pi * ph) + ev[
        mi, -1] * np.cos(2 * np.pi * ph), 'r-')
    plt.ylabel('Mag')
    plt.xlabel('Phase')
    ax = plt.gca()
    ax.set_xlim([0, 2])
    ...
    plt.scatter(phase[q2], dd[q2], s=2, edgecolor='', c=colour2,
                cmap=mpl.cm.winter)
    plt.scatter(phase[q2] + 1, dd[q2], s=2, edgecolor='', c=colour2,
                cmap=mpl.cm.winter)
    plt.plot(ph, np.mean(mags) + ev[mi, -2] * np.sin(2 * np.pi * ph) + ev[
        mi, -1] * np.cos(2 * np.pi * ph), 'r-')
    ...
    plt.subplot(5, 1, 4)
    plt.scatter(dates[q1], mags[q1], s=2, edgecolor='', c=colour1,
                cmap=mpl.cm.winter)
    plt.plot(tt,
             np.mean(mags) + ev[mi, -2] * np.sin(ttp) + ev[mi, -1] * np.cos(
                 ttp), 'r-')
    plt.ylabel('Mag')
    plt.xlabel('Date')
    ax = plt.gca()
    ...
    plt.subplot(5, 1, 5)
    plt.scatter(dates[q2], dd[q2], s=2, edgecolor='', c=colour2,
                cmap=mpl.cm.winter)
    plt.plot(tt,
             np.mean(mags) + ev[mi, -2] * np.sin(ttp) + ev[mi, -1] * np.cos(
                 ttp), 'r-')
...
def ddp_search(dates, data, err, pmin, pmax, nfreq, npc=10, nlc=None,
               BIC_threshold=7, recompute_eigen=True):
    from sklearn.decomposition import PCA
    from scipy import linalg
    if nlc:
        npts = data.shape[1]
    ...
    for k in np.arange(nlc):
        ...
        for i, f in enumerate(freq):
            ...
            try:
                ...
                lnprob[i] = np.log(2 * np.pi) - 0.5 * linalg.det(
                    W0) - 0.5 * np.dot(w[-2:].T,
                                       np.dot(U, w[-2:])) - 0.5 * chi2
                #                vs = U[0,0]
                #                vc = U[1,1]
                #                vsc =U[0,1]
                #                cs = w[-2]
        ...
        for i in range(5):
            p = np.where(
                abs(lnprob[p] - np.mean(lnprob[p])) < 3 * np.std(lnprob[p]))
        sn = (mb - np.mean(lnprob[p])) / np.std(lnprob[p])
        print
        'star', k, '  lnProb =', mb, '  S/N =', sn
        if sn > BIC_threshold:
            p = np.where(lnprob == mb)
            print
            'Detected period', 10.0 ** logp[
                p], 'for star', k, 'with lnProb', mb
            #            print 'Coefficients',w
            plt.figure(figsize=(8.27, 11.69), dpi=100)
            plotddp(k, dates, logp, sn, lnprob, data[k, :], p[0][0],
                    A * err[k, :].reshape(npts, 1), ev)
            plt.savefig('vard%(k)05d.pdf' % vars(), orientation='portrait',
                        papertype='a4')
            plt.close()
...
def slot_acf(t, x, x_err, delta_tau, n):
    ...
    for k in range(n):
        ...
        acf[k] = np.sum([np.dot(x_norm[m] * np.ones(nx), Xjb[:, m]) for m in
                         range(nx)]) / np.sum(b)
...
def gaussian_kernel_acf(t, x, x_err, delta_tau, n):
    ...
    for k in range(n):
        ...
        b = np.tril(b, k=-1)
        Xjb = Xj * b
        acf[k] = np.sum([np.dot(x[m] * np.ones(nx), Xjb[:, m]) for m in
                         range(nx)]) / np.sum(b)
