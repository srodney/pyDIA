...
import fnmatch
import numpy as np
from scipy.ndimage import label, center_of_mass, fourier_ellipsoid, \
    fourier_gaussian, gaussian_filter
...
def final_variable_photometry(files, params, coords=None, coord_file=None,
                              psf_file=None):
    if params.use_GPU:
    ...
    if not (os.path.exists(psf_file)):
        print
        'Error: PSF file', psf_file, 'not found'
        sys.exit(1)
    ...
    if coords == None:
        if coord_file:
            ...
            print
            'Error in final_variable_photometry: no coordinates provided.'
            sys.exit(1)
    ...
    if not (params.use_GPU):
        ...
        star_sort_index, star_group_boundaries, detector_mean_positions_x, detector_mean_positions_y = group_stars_ccd(
            params, coords, params.loc_output + os.path.sep + 'ref.fits')
        ...
        for i, f in enumerate(files):
            ...
            if os.path.exists(dfile) and os.path.exists(
                    nfile) and os.path.exists(ktable):
                kernelIndex, extendedBasis, c, params = read_kernel_table(
                    ktable, params)
        ...
        for i, f in enumerate(files):
            ...
            if os.path.exists(dfile) and os.path.exists(
                ...
                flux[i, :], dflux[i, :] = photom_all_stars(diff, inv_var,
                                                           coords, psf_file, c,
                                                           kernelIndex,
                                                           extendedBasis,
                                                           kernelRadius,
...
def detect_variables(params, psf_file=None, ccoords=None, time_sigma=4.0,
    ...
    if (ccoords == None):
        ...
        while j < len(filenames):
            ...
            print
            'len_filenames, j, stack_size', len(filenames), j, stack_size
            ...
            while (j < len(filenames)) and (nstack < stack_size):
                ...
                if os.path.exists(dfile) and os.path.exists(
                    ...
                    if np.nanstd(n) < params.diff_std_threshold:
                        ...
                        #
                        stackd[nstack, :, :], stackn[nstack, :,
                                              :] = convolve_image_with_psf( \
                            psf_file, d, n, c, kernelIndex, extendedBasis,
                            kernelRadius, params)
                        nstack += 1
            ...
            # Filter by time
            #
            stackn = gaussian_filter(stackn[:nstack, :, :],
                                     sigma=(time_sigma, 0.0, 0.0))
            ...
            for cen in centroids:
                ...
                if (x > dr) & (x < xs - dr) & (y > dr) & (y < ys - dr):
                    ...
                    if star_coords is None:
                        ...
                        if (x - star_coords[0]) ** 2 + (
                                y - star_coords[1]) ** 2 < star_coords[2] ** 2:
                            good_centroids.append(cen)
                    stackn_thresh = -stackn[:nstack, :, :].copy()
                    labeled_image, number_of_objects = label(stackn_thresh)
                    centroids = center_of_mass(stackn_thresh, labeled_image, \
        ...
        np.savetxt(params.loc_output + os.path.sep + 'variables.coords',
                   coords)
        ...
        if len(final_centroids) > 0:
            ...
            #
            print
            'Doing photometry at centroid positions'
            ...
            if not (params.use_GPU):
                star_sort_index, star_group_boundaries, detector_mean_positions_x, detector_mean_positions_y = group_stars_ccd(
                    params, coords,
                    params.loc_output + os.path.sep + 'ref.fits')
                scoords = coords[star_sort_index]
                star_unsort_index = np.argsort(star_sort_index)
                ...
                for i, f in enumerate(filenames):
                    ...
                    if os.path.exists(dfile) and os.path.exists(
                        ...
                        sflux, sdflux = photom_all_stars(diff, inv_var,
                                                         scoords, psf_file, c,
                                                         kernelIndex,
                                                         extendedBasis,
                                                         kernelRadius, params,
                ...
                for i, f in enumerate(filenames):
                    ...
                    if os.path.exists(dfile) and os.path.exists(
                        ...
                        flux[i, :], dflux[i, :] = photom_all_stars(diff,
                                                                   inv_var,
                                                                   coords,
                                                                   psf_file, c,
                                                                   kernelIndex,
            ...
            for i in range(len(final_centroids)):
                np.savetxt(
                    params.loc_output + os.path.sep + 'var%03d.flux' % i,
            ...
            ccoords = coords.copy()
            for i in range(len(final_centroids)):
                ccoords[i, :], flux, dflux = do_photometry_variables(filenames,
                                                                     params,
                                                                     coords[i,
                                                                     :],
                                                                     psf_file=psf_file)
                np.savetxt(
                    params.loc_output + os.path.sep + 'cvar%03d.flux' % i,
                    np.vstack((flux, dflux)).T)

            np.savetxt(params.loc_output + os.path.sep + 'variables.ccoords',
                       ccoords)
    ...
    print
    'Final photometry'
    flux, dflux = final_variable_photometry(filenames, params, coords=ccoords,
                                            psf_file=psf_file)
    ...
    ref = Observation(params.loc_output + os.path.sep + reference_image,
                      params)
    ref.image = ref.data
...
def do_photometry_variables(files, params, position, extname='vflux',
                            psf_file=None, reference_image='ref.fits',
                            iterations=10):
    #
    # Get image size
    #
    ...
    for j, f in enumerate(files):
        ...
        if os.path.exists(dtarget) and os.path.exists(
            ...
            kernelIndex, extendedBasis, c, params = read_kernel_table(ktable,
                                                                      params)
            diff[abs(diff) > 1.2 * params.pixel_max] = np.nan
            inv_var = (norm / diff) ** 2 + (1 - mask)
            diff[np.isnan(diff)] = 0.0
            diff = undo_photometric_scale(diff, c, params.pdeg, size=imsize,
                                          position=(xpos, ypos))
    ...
    while (ngood < 12) and (good_threshold > 3.0):
        ...
        for j, f in enumerate(files):
            snorm = np.std(nn[j, :, :])
            print
            'j,snorm, max = ', j, snorm, np.max(np.abs(nn[j, 14:17, 14:17]))
            if not (np.isinf(vv[j, :, :]).any() or np.isnan(
                    vv[j, :, :]).any() or np.isnan(dd[j, :, :]).any()) and (
                    np.nanstd(nn[j, :, :]) < params.diff_std_threshold) and (
                    np.max(np.abs(
                            nn[j, 14:17, 14:17])) > good_threshold * snorm):
                good[j] = True
                ngood += 1
        good_threshold *= 0.9
    ...
    for i in range(iterations):
        ...
        for j, f in enumerate(files):
            ...
            if os.path.exists(ktable) and good[j]:
                ...
                flux[j], dflux[
                    j], sjx1, sjx2, sjy1, sjy2 = ci.photom_variable_star(
                    dd[j, :, :], vv[j, :, :], position, position0, psf_file, c,
        ...
        if (position1[0] - position0[0]) ** 2 + (
                position1[1] - position0[1]) ** 2 > 25.0:
            position0 = position1
            print
            'Convergence failed'
