...
def locate_intercept(x, y, x_range):
    ...
    p = np.where(x[ix] > 2)[0]
    ix = ix[p]
    p = np.where(np.logical_and(x[ix] < np.percentile(x[ix], x_range[1]),
                                x[ix] > np.percentile(x[ix], x_range[0])))[0]
    ix = ix[p]
    for n in range(10):
        ...
        a, x.shape
        p = np.where(
            np.abs(y[ix] - a - x[ix]) < 2.5 * np.nanstd(y[ix] - a - x[ix]))[0]
        ix = ix[p]
    return a, ix


def calibrate(dir, plotfile='calibration.png',
              magnitude_range_fraction=(0.1, 8), sky_flux_cutoff_percent=0.1):
    ...
    offset, stars = locate_intercept(mag[p, 3],
                                     ZP - 2.5 * np.log10(flux[p, 0]),
                                     magnitude_range_fraction)
    ...
    ax1.scatter(mag[p, 3], ZP - 2.5 * np.log10(flux[p, 0]) - mag[p, 3], s=5,
                color='k')
    ax1.scatter(mag[p[stars], 3],
                ZP - 2.5 * np.log10(flux[p[stars], 0]) - mag[p[stars], 3], s=5,
                color='c')
    # ax.plot(x,x,'r--')
    ...
    hval, bins, _ = ax2.hist(mag[p, 3], range=(xmin, xmax),
                             bins=int((xmax - xmin) / binsize + 1),
    ...
    ax3.hist(ZP - 2.5 * np.log10(flux[p, 0]) - mag[p, 3], range=(ymin, ymax),
             bins=int((ymax - ymin) / binsize + 1), orientation='horizontal',
             normed=True, alpha=0.3)
    ...
    mag[:, 3] += offset
    if mag.shape[1] == 4:
        np.savetxt(os.path.join(dir, 'ref.mags.calibrated'), mag,
                   fmt='%5d  %8.3f  %8.3f  %7.4f')
    else:
        np.savetxt(os.path.join(dir, 'ref.mags.calibrated'), mag,
                   fmt='%5d  %8.3f  %8.3f  %7.4f  %7.4f  %7.3f  %7.3f %7.3f')
    ...
    np.savetxt(os.path.join(dir, 'ref.flux.calibrated'),
               np.vstack((cflux, cfluxerr)).T, fmt='%12.4f  %12.4f')


def makeCMD(dirI, dirV, bandwidth=0.25, ifile=None, vfile=None,
            plot_density=True, IV=None, RC=None, source_colour=None,
    ...
    if RC is not None:
        plt.scatter(RC[0], RC[1], color='r', marker='+', s=40,
                    label='Red Clump (%6.3f,%6.3f)' % RC)
    ...
    np.savetxt(dirI + '-' + dirV + '-CMDdata', np.vstack((im[p, 0], im[p, 1],
                                                          im[p, 2], vm[p, 3],
                                                          vm[p, 4], im[p, 3],
                                                          im[p, 4])).T,
    ...
    if plot_density:
        ...
        if np.sum(lm) > 0:
            ...
            Z[local_maxima] / Zmax
            print
            'Red clump detected at', red_clump
            plt.scatter(red_clump[0], red_clump[1], color='c', marker='+',
                        s=40,
                        label='Red Clump estimated at (%6.3f,%6.3f)' % red_clump)
        else:
            print
            'Error detecting red clump'
            red_clump = None
...
def source_colour(ifile, vfile, interval=0.05, plotfile='source_colour.png'):
    ...
    good_data = \
    np.where(np.logical_and(np.isfinite(flux1), np.isfinite(flux2)))[0]
    offset = np.mean(flux2[good_data] - flux1[good_data])

    # Create a RealData object using our initiated data from above.
    data = RealData(flux1[good_data], flux2[good_data],
                    sx=flux1_err[good_data], sy=flux2_err[good_data])
...
def plot_lightcurve(file, columns=(0, 3, 4), plotfile='lightcurve.png',
    ...
    plt.errorbar(data[:, columns[0]], data[:, columns[1]], data[:, columns[2]],
                 fmt='.')
    plt.gca().invert_yaxis()
