...
def convolve_image_with_psf(psf_image, image1, image2, c, kernelIndex,
    ...
    cu_convolve(np.int32(profile_type), np.int32(nx), np.int32(ny),
                ...
                np.int32(c.shape[0]), np.int32(kernelIndex.shape[0]),
                np.int32(kernelRadius), cuda.In(k0), cuda.In(k1),
                cuda.In(extendedBasis), cuda.In(psf_parameters),
                cuda.In(psf_0), cuda.In(psf_xd), cuda.In(psf_yd), cuda.In(c),
                cuda.Out(convolved_image1), cuda.Out(convolved_image2),
